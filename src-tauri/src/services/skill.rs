...
//!
//! v3.10.0+ 统一管理架构：
//! - SSOT（单一事实源）：`~/.codexpro-tool/skills/`
...
pub enum SkillStorageLocation {
    /// CC Switch 管理目录 (~/.codexpro-tool/skills/)
    #[default]
    CcSwitch,
...
impl SkillService {
    ...
    /// 获取 SSOT 目录（根据设置返回 ~/.codexpro-tool/skills/ 或 ~/.agents/skills/）
    ...
    /// 获取 Skill 卸载备份目录（~/.codexpro-tool/skill-backups/）
    ...
    fn resolve_uninstall_backup_source_excluding(
        ...
        // 返回值会被整目录复制进 ~/.codexpro-tool/skill-backups/ 并由 get_skill_backups
...
mod tests {
    ...
    fn restore_from_backup_rejects_traversal_directory_in_metadata() {
        ...
        // SSOT 位于 {home}/.codexpro-tool/skills，"../../pwned-restore" 若生效会写到 {home}/pwned-restore。
    ...
    fn uninstall_rejects_traversal_directory_from_db_row() {
        ...
        // 与 import_sql_string_for_sync 的效果一致）。SSOT = {home}/.codexpro-tool/skills，
    ...
    fn migrate_storage_retargets_an_equivalent_relative_pi_symlink() {
        ...
        std::os::unix::fs::symlink(Path::new("../../.codexpro-tool/skills/test-skill"), &pi_skill)
