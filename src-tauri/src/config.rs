...
///   且不一定等于用户目录，可能导致 `.codexpro-tool/codexpro-tool.db` 路径变化，从而“看起来像数据丢失”。
///
/// ## 测试隔离
///
...
/// 获取应用配置目录路径 (~/.codexpro-tool)
///
/// CodexPRO Tool 与 CC Switch 独立共存：绝不回退/复用 `~/.cc-switch`。
pub fn get_app_config_dir() -> PathBuf {
    if let Some(custom) = crate::app_store::get_app_config_dir_override() {
        return custom;
    }

    get_home_dir().join(".codexpro-tool")
}
