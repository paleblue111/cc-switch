...
fn handle_deeplink_url(
    ...
    if !url_str.starts_with("codexpro://") {
        return false;
    }
...
pub fn run() {
    // 设置 panic hook，在应用崩溃时记录日志到 <app_config_dir>/crash.log（默认 ~/.codexpro-tool/crash.log）
    ...
    let builder = builder
        ...
        .setup(|app| {
            ...
            // 初始化日志（输出到 <app_config_dir>/logs/codexpro-tool.log）
            {
                    ...
                    tauri_plugin_log::Builder::default()
                        ...
                        .targets([
                            ...
                            Target::new(TargetKind::Folder {
                                path: log_dir,
                                file_name: Some("codexpro-tool".into()),
                            }),
                        ])
                ...
                log::info!("=== CodexPRO Tool v{} started ===", env!("CARGO_PKG_VERSION"));
            }
            ...
            let db_path = app_config_dir.join("codexpro-tool.db");
            ...
            #[cfg(any(target_os = "linux", all(debug_assertions, windows)))]
            {
                #[cfg(target_os = "linux")]
                {
                    ...
                    // tauri-plugin-deep-link writes to: ~/.local/share/com.codexpro.tool/applications/codexpro-tool-handler.desktop
                ...
                move |event| {
                    ...
                    for (i, url) in urls.iter().enumerate() {
                        ...
                        if handle_deeplink_url(&app_handle, url_str, true, "on_open_url") {
                            break; // Process only first codexpro:// URL
                        }
                    }
                }
            });
            ...
            // 构建托盘
            // 左键：显示/聚焦主窗口（不弹菜单）；右键：上下文菜单（Show + Quit）
            let mut tray_builder = TrayIconBuilder::with_id(tray::TRAY_ID)
                .tooltip("CodexPRO Tool") // 鼠标悬停提示
                .on_tray_icon_event(|tray, event| match event {
                    // 鼠标悬停/点击到托盘图标时，后台异步刷新用量缓存，
                    // 让用户下一次（或快速打开菜单的那一刻）看到较新的数字。
                    // refresh_all_usage_in_tray 内部有 10 秒防抖；不阻塞 show。
                    TrayIconEvent::Enter { .. } => {
                    ...
                    TrayIconEvent::Click {
                        ...
                        // Prefer Up to avoid double-fire with Down on some platforms.
    ...
    app.run(|app_handle, event| {
        ...
        #[cfg(target_os = "macos")]
        {
            match event {
                ...
                // 处理通过自定义 URL 协议触发的打开事件（例如 codexpro://...）
...
fn show_migration_error_dialog(app: &tauri::AppHandle, error: &str) -> bool {
    ...
    let message = if is_chinese_locale() {
        ...
        format!(
            ...
            Consider rolling back to an older CodexPRO Tool version.\n\n\
...
fn show_database_init_error_dialog(
    ...
    let message = if is_chinese_locale() {
        format!(
            ...
            建议：\n\
            1) 先备份整个配置目录（包含 codexpro-tool.db）\n\
        ...
        format!(
            ...
            Suggestions:\n\
            1) Back up the entire config directory (including codexpro-tool.db)\n\
            2) If you see “database version is newer”, please upgrade CodexPRO Tool\n\
